...
.get_records <- function(x, sleep) {
  ...
  sum0 <- textshape::tidy_list(
    x = lapply(parsed_records, "[[", 1),
    id.name = "id",
    content.name = "varx"
  )

  # Reshape the data into a wide format using data.table
  sum1 <- data.table::dcast(
    data = sum0,
...
.extract_annotations <- function(g) {
  ...
  df0 <- rbind(
    data.frame(pmid = pm, type = "MeSH", form = if (length(meshes) > 0) {
      meshes
    } else {
      NA
    }),
    data.frame(pmid = pm, type = "Chemistry", form = if (length(chems) > 0) {
      chems
    } else {
      NA
    }),
    data.frame(pmid = pm, type = "Keyword", form = if (length(keys) > 0) {
      keys
    } else {
      NA
    })
