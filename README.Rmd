...
PubMed Article Records: Offers comprehensive records of scientific articles from the MEDLINE database, including titles, abstracts, and publication details, serving as a primary research resource.

PubMed Author Affiliations: Details institutional and geographic affiliations of authors in published articles, useful for analyzing research networks and collaborations.

PMC Full-Text Access: Enables direct access to full-text articles from PubMed Central, enhancing research with comprehensive content analysis and data mining capabilities.

iCite Database: Offers bibliometric analysis with citation metrics and Relative Citation Ratio (RCR) for scientific publications, aiding in the assessment of research impact.

PubMed Annotations (PubTator): Provides tagged entities like genes and diseases in article texts, crucial for bioinformatics research, via the PubTator service.
...
PubMedToolkit is an R package specifically designed for streamlined and efficient access to various PubMed-related databases and services. It caters to the needs of researchers, bioinformaticians, and data scientists in biomedical fields, providing a single interface for diverse data retrieval and analysis tasks.

Key Features:
With a vector of PMIDs ... (not quite) -- `pubmedtk` ... designed for simple, uniform access 
uniform access to multiple text-based PubMed resources/DBs, including: PubMed abstract records, PubMed author affiliation data, PMC full-texts, iCite citation/bibliometric data, and PubTator NER annotations.  


Efficient Data Retrieval and Processing:

Facilitates batch retrieval of data using functions like pmed_get_data.
Employs parallel processing for enhanced performance, ideal for handling large datasets and complex queries.


Broad Application Spectrum:

Suited for systematic literature reviews, meta-analyses, network analysis, text mining, and impact assessment studies.
...
med_cannabis <- med_cannabis |> unlist() |> unique() |> sort()
```
...
```{r}
pubmed_data <- med_cannabis |> pubmedtk::pmed_get_data(task_type = 'pubmed')
pubmed_affs <- med_cannabis |> pubmedtk::pmed_get_data(task_type = 'affiliations')
icites_data <- med_cannabis |> pubmedtk::pmed_get_data(task_type = 'icites')
pubtations_data <- med_cannabis |> pubmedtk::pmed_get_data(task_type = 'pubtations')
```
